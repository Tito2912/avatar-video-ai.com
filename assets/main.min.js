...
qsa('[data-ev]').forEach(el=>{on(el,'click',e=>{const name=el.getAttribute('data-ev');const label=el.getAttribute('data-label')||el.getAttribute('href')||el.textContent?.trim();sendEvent(name,{label});});});qsa('a[rel~="sponsored"]').forEach(a=>{const href=a.getAttribute('href')||'';if(!/heygen\.com/i.test(href))return;const ev=(a.getAttribute('data-ev')||'').toLowerCase();if(ev==='affiliate_click')return;on(a,'click',()=>{const label=a.getAttribute('data-label')||href;sendEvent('affiliate_click',{label});});});(function trackScroll50(){let fired=false;function handler(){if(fired)return;const scrolled=(window.scrollY+window.innerHeight)/document.documentElement.scrollHeight;if(scrolled>=0.5){fired=true;sendEvent('scroll_50');off(window,'scroll',handler);}}
